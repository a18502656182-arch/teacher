...
当前入口为 `docs/逐页设计定稿与实施包-20260914/00-从这里开始.md`。每页独立完成桌面、手机、关键状态候选与可运行设计稿；每批先取得D设计批准，再接入业务，完成后取得R实际批准才进下一批。首页历史批准不替代其它页设计。监工改为可选技术检查，下文强制监工PASS/母版延展的旧流程不再适用；历史报告保留。主账本与脚本分阶段迁移按新包05实施，不伪造旧PASS。当前从V2设计接续，保留既有首页批准和未提交学生页改动。授权制作隔离设计稿不等于授权未经设计批准改正式接线。
