...
test("benchmark pages use reference-led compositions and semantic artwork slots", () => {
  ...
  assert.match(dashboard, /classroom-morning-wide\.webp[\s\S]*objectFit: 'cover'/);
  ...
  assert.doesNotMatch(studentView, /<th>家长电话<\/th>|<th>备注<\/th>/);
...
test("attendance keeps all actions in the batch-capable main roster", () => {
  ...
  assert.match(attendancePage, /Attendance\.module\.css/);
  assert.match(attendancePage, /data-ui-generation="next"/);
  assert.match(attendanceStyles, /\.batchBar/);
  assert.match(attendancePage, /async function applyBatch\(\)[\s\S]*?if \(ok\) \{ setSelectedIds\(\[\]\)/);
  assert.match(attendancePage, /async function setStudentStatus\([\s\S]*?if \(ok\) \{ setSelectedIds\(\[\]\)/);
  assert.match(attendancePage, /同步失败，本机改动和当前选择已保留，请重试/);
