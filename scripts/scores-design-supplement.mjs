...
const out = path.resolve(process.env.DESIGN_CAPTURE_DIR || '.qa-shots/scores-design/supplement');
mkdirSync(out,{recursive:true});
const chrome = spawn('C:/Program Files/Google/Chrome/Application/chrome.exe',['--headless=new','--disable-gpu','--no-first-run','--no-default-browser-check','--remote-debugging-port=9256',`--user-data-dir=${path.resolve('.qa-shots/scores-design/supplement-chrome')}`,'about:blank'],{stdio:'ignore',windowsHide:true});
...
try {
 let tabs; for(let i=0;i<40;i++){try{tabs=await fetch('http://127.0.0.1:9256/json').then(r=>r.json());break;}catch{await wait(250);}}
 ...
 const snap=async(name,width)=>{await evaluate('document.fonts.ready.then(()=>true)');const info=await evaluate(`(()=>{const selectors={page:'[data-scores-design]',heading:'[data-scores-design] h1',search:'[aria-label="搜索成绩学生"]',row:'[data-score-row]',notice:'.score-trends-chart',dialog:'dialog[open]'};return{scrollWidth:document.documentElement.scrollWidth,regions:Object.fromEntries(Object.entries(selectors).map(([k,s])=>{const e=document.querySelector(s);if(!e)return[k,null];const r=e.getBoundingClientRect(),c=getComputedStyle(e);return[k,{x:r.x,y:r.y,width:r.width,height:r.height,font:c.font,padding:c.padding,gap:c.gap,background:c.backgroundColor}]}))}})()`);report.measurements.push({width,name,...info});if(info.scrollWidth>width+1)report.failures.push({name,width,overflow:info.scrollWidth});const bytes=Buffer.from((await send('Page.captureScreenshot',{format:'png'})).data,'base64');writeFileSync(path.join(out,name+'.png'),bytes);report.evidence.push({path:name+'.png',sha256:hash(bytes),width});};
 const go=async(state,width)=>{await send('Emulation.setDeviceMetricsOverride',{width,height:width<900?844:1024,deviceScaleFactor:1,mobile:width<900});await send('Page.navigate',{url:'http://127.0.0.1:4218/scores.html?state='+state});await wait(1200);};
 ...
 for(const width of [1536,390,1057,360]){await go('normal',width);await snap(width+'-normal',width);const doc=await send('DOM.getDocument');const node=await send('DOM.querySelector',{nodeId:doc.root.nodeId,selector:'[data-scores-design] h1'});report.measurements.at(-1).platformFonts=await send('CSS.getPlatformFontsForNode',{nodeId:node.nodeId});}
 for(const width of [1536,390]){
 await go('trends',width);await evaluate("document.querySelector('.score-trends-heading-actions button').click()");await wait(150);await snap(width+'-student-picker',width);await check(width+' picker present',"document.body.innerText.includes('选择趋势学生')");
 await go('conflict-409',width);await evaluate("[...document.querySelectorAll('dialog[open] button')].find(b=>b.textContent==='导出草稿').scrollIntoView({block:'center'})");await snap(width+'-recovery',width);await evaluate("HTMLAnchorElement.prototype.click=function(){window.__exportName=this.download};[...document.querySelectorAll('dialog[open] button')].find(b=>b.textContent==='导出草稿').click()");await check(width+' export draft invocation',"window.__exportName==='synthetic-scores-draft.json'");await evaluate("[...document.querySelectorAll('dialog[open] button')].find(b=>b.textContent==='载入最新数据').click()");await wait(120);await check(width+' explicit replace confirmation',"document.body.innerText.includes('确认载入最新')");await evaluate("[...document.querySelectorAll('button')].find(b=>b.textContent==='确认载入最新').click()");await wait(1200);await check(width+' replace latest reload',"location.search==='?state=normal'&&!window.__scores.controller.pending");
 }
