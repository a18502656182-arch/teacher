...
const out = path.resolve(process.env.DESIGN_CAPTURE_DIR || 'docs/page-designs/V4/scores/screenshots/p02-controls');
...
for(const width of [1536,390,360]){await go('long-history',width);await evaluate("document.querySelector('[aria-label=\"趋势考试范围\"]').focus()");await send('Input.dispatchKeyEvent',{type:'keyDown',key:'ArrowDown',code:'ArrowDown',windowsVirtualKeyCode:40});await send('Input.dispatchKeyEvent',{type:'keyUp',key:'ArrowDown',code:'ArrowDown',windowsVirtualKeyCode:40});await wait(120);await check(width+' keyboard range changes',"document.querySelector('[aria-label=\"趋势考试范围\"]').value==='24'&&document.querySelector('[data-trends-view]').textContent.includes('/24场')");await check(width+' readable control size',"(()=>{const e=document.querySelector('[aria-label=\"趋势考试范围\"]'),s=getComputedStyle(e);return parseFloat(s.fontSize)>=14&&e.getBoundingClientRect().height>=36&&parseFloat(s.paddingRight)>=30})()");await check(width+' no overflow',"document.documentElement.scrollWidth<=innerWidth+1");const shot=await send('Page.captureScreenshot',{format:'png',captureBeyondViewport:false});const bytes=Buffer.from(shot.data,'base64'),name=width+'-keyboard.png';writeFileSync(path.join(out,name),bytes);report.evidence.push({path:name,sha256:hash(bytes)});}console.log(report.interactions);
}catch(e){report.failures.push(String(e));console.error(e);process.exitCode=1;}finally{writeFileSync(path.join(out,'controls-interactions.json'),JSON.stringify(report,null,2)+'\n');ws?.close();chrome.kill();}