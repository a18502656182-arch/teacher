...
export function Attendance({ data, update, mobile = false }: { data: ClassroomData; update: (fn: (data: ClassroomData) => ClassroomData) => void; mobile?: boolean }) {
  ...
  return <section className={`attendance-page attendance-ledger ${mobile ? "attendance-page-mobile" : ""}`}>
    ...
    {mobile && <header className="attendance-mobile-head"><p>选择日期，登记当天考勤。</p><label>日期<input type="date" value={selectedDate} onChange={(event) => { setSelectedDate(event.target.value); setMonth(event.target.value.slice(0, 7)); }} /></label></header>}
    <section className="campus-statistics" aria-label="当天考勤概览"><span className="normal"><small>正常到校</small><b>{counts.正常}</b></span><span className="late"><small>迟到</small><b>{counts.迟到}</b></span><span className="leave"><small>请假</small><b>{counts.请假}</b></span><span className="absent"><small>缺勤</small><b>{counts.缺勤}</b></span></section>
    ...
    <section className="attendance-roster">
      ...
      <div className="attendance-student-list">{pageStudents.map((student) => {
        ...
        return <article key={student.id} className={`attendance-student ${selectedIds.includes(student.id) ? "selected" : ""}`}><label className="attendance-row-select"><input type="checkbox" aria-label={`选择${student.name}`} checked={selectedIds.includes(student.id)} onChange={() => toggleStudent(student.id)} /><span aria-hidden="true" /></label><div className="attendance-student-main"><i aria-hidden="true">{student.name.slice(0, 1)}</i><span><b>{student.name}</b><small>第{student.group}组 · {student.studentNo || "未编学号"}</small></span></div><div className="attendance-status-actions" aria-label={`${student.name}的考勤状态`}>{attendanceStatuses.map((item) => <button type="button" key={item} aria-pressed={status === item} className={`${statusTone[item]} ${status === item ? "active" : ""}`} onClick={() => setStudentStatus(student.id, item)}>{item}</button>)}</div><label className="attendance-row-note"><span className="sr-only">{student.name}的当日备注</span><input value={note} onChange={(event) => setNoteDrafts((current) => ({ ...current, [student.id]: event.target.value }))} placeholder="补充到校时间、异常原因或跟进事项" /></label><button type="button" className="attendance-save-note" onClick={() => saveStudentNote(student.id)}>保存</button></article>;
