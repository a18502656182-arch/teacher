...
function linePath(points: TrendPoint[]) {
  const usable = points.map((point, index) => ({ value: point.value, x: points.length === 1 ? 50 : 8 + index / (points.length - 1) * 84 })).filter((point): point is { value: number; x: number } => point.value != null);
  if (!usable.length) return "";
  return usable.map((point, index) => `${index ? "L" : "M"}${point.x} ${92 - point.value * .78}`).join(" ");
}
...
export function ScoreTrends({ data, classId }: { data: ClassroomData; classId: string }) {
  ...
  return <section className="score-trends score-trends-workbench" aria-label="历次成绩趋势">
    ...
    {points.length < 2 ? <div className="score-trends-empty">至少需要两场已保存考试，才能显示纵向变化；当前仍可在本次成绩中处理学生跟进。</div> : <>
      <section className="score-trends-summary" aria-label="趋势摘要"><article><span>最近得分率</span><b>{latest == null ? "未录入" : `${latest}%`}</b><small>{student ? student.name : "班级整体"}</small></article><article><span>相对上次</span><b className={delta == null ? "neutral" : delta > 0 ? "up" : delta < 0 ? "down" : "neutral"}>{delta == null ? "数据不足" : `${delta > 0 ? "+" : ""}${delta} 个百分点`}</b><small>仅比较相邻两场均有录入的数据</small></article><article><span>可用记录</span><b>{recorded.length}/{points.length}</b><small>未录入的场次不会连接为零分</small></article></section>
      <section className="score-trends-chart" aria-label="得分率折线图"><div><h3>{student ? `${student.name}的得分率变化` : "班级整体得分率变化"}</h3><span>{points.length > chartPoints.length ? `图表展示最近 ${chartPoints.length} 场；完整记录见下方台账` : "横轴按时间，纵轴为得分率"}</span></div><svg viewBox="0 0 100 100" role="img" aria-label="得分率随考试时间变化的折线图" preserveAspectRatio="none"><line x1="8" x2="92" y1="14" y2="14" /><line x1="8" x2="92" y1="53" y2="53" /><line x1="8" x2="92" y1="92" y2="92" />{path && <path d={path} />}{chartPoints.map((point, index) => point.value == null ? null : <circle key={point.exam.id} cx={chartPoints.length === 1 ? 50 : 8 + index / (chartPoints.length - 1) * 84} cy={92 - point.value * .78} r="1.6"><title>{`${point.exam.title}：${point.value}%`}</title></circle>)}</svg><div className="score-trends-axis"><span>{chartPoints[0]?.exam.date || ""}</span><span>{chartPoints.at(-1)?.exam.date || ""}</span></div></section>
