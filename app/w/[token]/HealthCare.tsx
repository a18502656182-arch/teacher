...
export function HealthCare({ data, update, save: persist, readOnly, mobile = false }: { data: ClassroomData; update: (recipe: (current: ClassroomData) => ClassroomData) => void; save: () => Promise<boolean>; readOnly: boolean; mobile?: boolean }) {
  ...
  const [draftBaseline, setDraftBaseline] = useState<CareDraft | null>(null);
  ...
  const dirty = Boolean(draft && draftBaseline && JSON.stringify(draft) !== JSON.stringify(draftBaseline));
  useEffect(() => {
    if (!draft || (!dirty && !busy)) return;
    const guard = (event: Event) => { event.preventDefault(); setEditorError("还有未保存的照护登记，请先保存或取消。"); };
  ...
  async function saveEntry() {
    ...
    if (readOnly) { setEditorError("当前为只读模式，不能保存照护登记。"); return; }
    const preview = saveCareProfile(data, activeClassId, draft, () => "care-preview");
    if (preview.error) {
      setEditorError(preview.error);
      return;
    }
    setBusy(true); setEditorError("");
    update((current) => saveCareProfile(current, activeClassId, draft, makeId).data ?? current);
    const ok = await persist();
    setBusy(false);
    if (ok) { setDraft(null); setDraftBaseline(null); }
    else setEditorError("同步失败，当前照护内容已保留，请重试。");
  }
  async function remove() { if (!draft?.id || readOnly || busy || !await requestCareConfirm("这条照护登记会从当前班级移除，且无法恢复。")) return; setBusy(true); update((current) => removeCareProfile(current, activeClassId, draft.id)); const ok = await persist(); setBusy(false); if (ok) { setDraft(null); setDraftBaseline(null); } else setEditorError("删除同步失败，本机草稿仍保留，可重试保存。"); }
  const newEntry = () => { if (readOnly) return; const next = emptyDraft(); setDraft(next); setDraftBaseline(next); setEditorError(""); };
  const openEntry = (item: CareProfile) => { const next = { ...item, summary: item.summary ?? "", instruction: item.instruction ?? "", contraindication: item.contraindication ?? "", actionContexts: item.actionContexts ?? [], customCategory: item.customCategory ?? "" }; setDraft(next); setDraftBaseline(next); setEditorError(""); };
  return <main className={`health-care-page ${mobile ? "health-care-mobile" : ""}`} data-health-page>
    {mobile ? <div className="health-care-mobile-intro"><span className="module-icon health"><CampusIcon name="health"/></span><div><h1>健康与照护</h1><p>最小必要信息 · 仅班主任可见</p></div><button className="primary-button" disabled={readOnly} onClick={newEntry}>新增登记</button></div> : <WorkbenchPageHeader icon="🩺" tone="coral" title="健康与照护" description="为日常安排保留必要行动提示；不展示诊断细节，也不作为公开标签。" meta={`${activeClass?.name ?? "当前班级"} · ${students.length} 名学生`} actions={<button className="primary-button" disabled={readOnly} onClick={newEntry}>新增照护登记</button>} />}
    <section className="campus-statistics" aria-label="照护概览"><article><span>已登记学生</span><strong>{coveredCount}</strong><small>当前班级 {students.length} 名学生中的必要提示</small></article><article><span>需优先留意</span><strong>{priorityCount}</strong><small>仅用于安排前核对，不替代医疗判断</small></article></section>
    <section className="health-care-privacy"><CampusIcon name="health"/><div><strong>隐私边界</strong><p>仅记录日常安排所需的行动提示；涉及诊疗、证明或敏感原件，请按学校制度线下保管。</p></div></section>
...
function CareEditor({ draft, students, guardian, readOnly, busy, error, onChange, onClose, onSave, onRemove }: { draft: CareDraft; students: Student[]; guardian?: Guardian; readOnly: boolean; busy: boolean; error: string; onChange: (draft: CareDraft) => void; onClose: () => void; onSave: () => void; onRemove: () => void }) {
